...
export default function LandingView({ onStart, isUploading = false, error = '' }) {
  const [repoURL, setRepoURL] = useState('');
  const [repoFile, setRepoFile] = useState(null);
  const fileInputRef = useRef(null);

  const hasInput = repoURL.trim() !== '' || repoFile !== null;
  const isDisabled = !hasInput || isUploading;
    ...
    <div className="flex-1 relative flex flex-col items-center justify-center overflow-hidden px-6 min-h-[calc(100vh-56px)]">
      ...
      >
        ...
        <div className="w-full max-w-md flex flex-col gap-4 mt-4 bg-surface-container-low/40 backdrop-blur-xl p-8 rounded-2xl border border-outline-variant/30 shadow-2xl ring-1 ring-white/5">
          ...
          >
            {isUploading ? 'Preparing Repository...' : 'Index a Repository'}
          ...
          <div className="flex flex-col sm:flex-row w-full gap-3">
            ...
            <button
              ...
              className="bg-transparent text-on-surface border border-outline-variant/60 hover:bg-surface-container hover:border-outline transition-colors rounded-lg px-4 py-3 flex items-center justify-center gap-2 text-sm font-medium whitespace-nowrap group cursor-pointer"
            >
              <FolderOpen className="w-4 h-4 text-outline group-hover:text-primary transition-colors" />
              {repoFile ? 'Zip Ready' : 'Upload'}
            </button>
          </div>

          {repoFile ? (
            <p className="text-xs text-on-surface-variant text-left">
              Selected zip: <span className="text-primary">{repoFile.name}</span>
            </p>
          ) : null}
